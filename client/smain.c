...
int tcp_senddim;
int hit_coord[2];
struct box match_map[10][10]; // mappa di lavoro
struct box enemy_map[10][10];
struct sockaddr_in server;
struct sockaddr_in client;
struct sockaddr_in enemy;
struct sockaddr_in work;// controllo che l'avversario mi parli su udp e non altri
u_int16_t udp_port; // quella su cui rimanere in ascolto
...
int close_conn=0;// per la chiusura del while del client
int shell=0;
char client_name[30];
char enemy_name[30];
int  udp_message[3]; // si mandano messaggi sempre della stessa dimensione! [0]=cmd_udp [1]/[2]=x/y (coordinate)
void* tcp_message;
void* msg_buffer;
